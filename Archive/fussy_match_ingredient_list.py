...
from Helpers.recipe_dataset_gen import get_ingre_list_from_dataset
...
def get_all_ingredient_mapping(all_ingredients_list):
    all_mapping = []
    food_code_dataset = get_food_code_dataset()
    min_score_threshold=50
    test_matchMode= "Best_Match"
    ...
    for eachIngre in all_ingredients_list:
        ...
        ingredient_match = get_match_food_codes(eachIngre, 
                                                        food_code_dataset,
                                                        min_score_threshold,
                                                        test_matchMode)
        ...
        if match:
            ...
            #map_created, untri_unit_map = get_save_nutrient_to_file(match,eachIngre,untri_unit_map)
...
'''For Testing'''
def test_with_ingre_list():
    sample_size=10
    processed_init_filename = f"./datasets/processed_recipes_init_{sample_size}.json"
    all_ingredients_list = get_ingre_list_from_dataset(processed_init_filename)
    get_all_ingredient_mapping(all_ingredients_list)
    #print(all_ingredients_list)
    

#test_with_ingre_list()